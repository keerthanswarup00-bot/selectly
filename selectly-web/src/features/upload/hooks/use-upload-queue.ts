...
export function useUploadQueue({ studioId, projectId, onComplete, onFileComplete }: UseUploadQueueOptions) {
  ...
  const processQueue = useCallback(async () => {
    ...
    onComplete?.(results)
  }, [isProcessing, studioId, projectId, onComplete, onFileComplete])

  async function processItem(item: UploadFileItem): Promise<"success" | "failed"> {
    ...
    try {
      ...
      for (let attempt = 0; attempt < config.upload.maxRetries; attempt++) {
        const result = await uploadImage(blob, item.filename, studioId, projectId)
        if (result.success) {
